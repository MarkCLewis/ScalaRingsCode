...
object AutomateOccultations extends App {
  ...
  val simulations = Vector(
    // TODO - Add simulations.  Possibly read from directory structure.
    Simulation("", 70000, 1e-6, 0.5, 0.5)
  )
  ...
  for (sim <- simulations) {
    val particles = data.CartAndRad.read(new File(sim.filename))
    val binned = SynthOccultations.binParticles(particles)
    val beamSize = 0.01/sim.r0
    val cutSpread = 50/sim.r0
    val (zmin, zmax) = {
      val sorted = particles.map(_.z).sorted
      (sorted(100), sorted(sorted.length - 100))
    }
    for (star <- stars; if sim.r0 >= star.rmin && sim.r0 <= star.rmax) {
      val poissonDist = new PoissonDistribution(star.i0 / 1000)
      val cutTheta = 0.0  // Currently radial
      val phi = (star.phiMin + star.phiMax)*0.5  // Decide how to pick this better
      val scanLength = (star.rmax-star.rmin)/star.duration // Length in km for a second
      val scans = SynthOccultations.multipleCuts(0, 0, phi, star.B, cutTheta, scanLength,
        0.0, beamSize, zmax-zmin, binned, poissonDist.sample, cutSpread)
      // TODO - Output something for the scans.
    }
  }
}