...
object SynthOccultations {
  case class BinData(xmin: Double, xmax: Double, ymin: Double, ymax: Double, binSize: Double, bins: IndexedSeq[IndexedSeq[IndexedSeq[Particle]]]) {
    lazy val zmin = bins.view.map(r => r.view.map(c => c.view.map(_.z).min).min).min
    lazy val zmax = bins.view.map(r => r.view.map(c => c.view.map(_.z).max).max).max
    lazy val maxRadius = bins.view.map(r => r.view.map(c => c.view.map(_.rad).max).max).max
  ...
  def rayBinIntersect(r: Ray, xbin: Int, ybin: Int, binData: BinData): Boolean = {
    ...
    if(xEnter < yExit && xEnter < xExit) {
      ...
      if(zEnter < binData.bins(xbin)(ybin).last.z && zExit > binData.bins(xbin)(ybin).head.z) {
        val enterIndex = firstParticleTouchingZ(zEnter, binData.bins(xbin)(ybin), binData.maxRadius)
        val exitIndex = lastParticleTouchingZ(zExit, binData.bins(xbin)(ybin), binData.maxRadius)
        (enterIndex to exitIndex).exists(i => rayParticleIntersect(r, binData.bins(xbin)(ybin)(i)))
      } else false
    } else false
  }
